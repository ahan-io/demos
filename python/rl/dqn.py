...
class DQN:
    ...
    def __init__(self, NUM_STATES, ENV_A_SHAPE, NUM_ACTIONS):
    ...
    def choose_action(self, state):
        ...
        state = torch.unsqueeze(k, 0)  # get a 1D array
        if np.random.randn() <= EPISILO:  # greedy policy
            action_value = self.eval_net.forward(state)
            action = torch.max(action_value, 1)[1].data.numpy()
            action = (
                action[0] if self.ENV_A_SHAPE == 0 else action.reshape(self.ENV_A_SHAPE)
            )
        else:  # random policy®
            action = np.random.randint(0, self.NUM_ACTIONS)
            action = (
                action if self.ENV_A_SHAPE == 0 else action.reshape(self.ENV_A_SHAPE)
            )
        return action
